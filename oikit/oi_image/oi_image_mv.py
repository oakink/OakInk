...
class OakInkImageMV:
    ...
    def _get_info_list(data_dir, split_key, data_split):
        if data_split == "train+val":
            info_list = json.load(open(os.path.join(data_dir, "image", "anno_mv", "split", split_key, "seq_train.json")))
        elif data_split == "train":
            info_list = json.load(
                open(os.path.join(data_dir, "image", "anno_mv", "split_train_val", split_key, "example_split_train.json"))
            )
        elif data_split == "val":
            info_list = json.load(
                open(os.path.join(data_dir, "image", "anno_mv", "split_train_val", split_key, "example_split_val.json"))
            )
        else:  # data_split == "test":
            info_list = json.load(open(os.path.join(data_dir, "image", "anno_mv", "split", split_key, "seq_test.json")))
        return info_list
